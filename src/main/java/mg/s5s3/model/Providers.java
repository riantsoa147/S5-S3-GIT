...
public class Providers {
    ...
    private String name;
    public Providers(){}
    public Providers(String name,Connection con) throws Exception{
        setName(name); 
    }
    public int getId() {
        return id;
    ...
    public static Providers getById(int id, Connection con) throws Exception {
    ...
    public static Providers[] getAll() throws Exception {
        Connection con = Database.getConnection();
        PreparedStatement st = null;
        ResultSet rs = null;
        List<Providers> items = new ArrayList<>();

        try {
            String query = "SELECT * FROM providers order by id asc ";
            st = con.prepareStatement(query);
            rs = st.executeQuery();
        ...
        return items.toArray(new Providers[0]);
    }
    public int insert(Connection con) throws Exception {
        PreparedStatement st = null;
        ResultSet rs = null;
        try {
            String query = "INSERT INTO providers (name) VALUES (?) RETURNING id";
            st = con.prepareStatement(query);
